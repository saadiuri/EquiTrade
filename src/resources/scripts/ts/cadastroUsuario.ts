...
async function handleCadastroUsuario(event: Event) {
    ...
    try {
        ...
        if (response.ok) {
            showMessage("Cadastro realizado com sucesso! Redirecionando para login...", true);
        } else {
            showMessage(`Erro: ${dados.message || dados.error || 'Falha no cadastro'}`, false);
        }

    } catch (error) {
        console.error("Erro ao cadastrar:", error);
        showMessage("Erro de conexão. Verifique se o servidor está rodando.", false);
    }
}
